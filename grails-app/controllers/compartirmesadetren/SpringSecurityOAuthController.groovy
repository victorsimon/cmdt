...
import compartirmesadetren.OAuthID
            ...
            boolean linked = command.validate() && User.withTransaction { status ->
                ...
                if (user) {
                    ...
                    if (user.validate() && user.save()) {
						if (oauthID.validate() && oauthID.save()) {
							oAuthToken = updateOAuthToken(oAuthToken, user)
							return true
						} else {
							command.errors.rejectValue("username", "invalid oauthid " + oauthID.errors)
						}
                    }
                } else {
...
class OAuthCreateAccountCommand {
    ...
    static constraints = {
        ...
        password1 blank: false, minSize: 4, maxSize: 64, validator: { password1, command ->
            if (command.username && command.username.equals(password1)) {
                return 'OAuthCreateAccountCommand.password.error.username'
            }

            if (password1 && password1.length() < 4 && password1.length() >= 64) {
